<template>
  <div class="test">
    ...
    <div>
      <b-table small hover striped :items="adminList" :fields="fields" style="color: LightGray; text-align: center">
...
export default {
  ...
  computed: {
    ...
    insertedResult() {
      return this.$store.getters.AdminInsertedResult
    },
    updatedResult() {
      return this.$store.getters.AdminUpdatedResult
    },
    deletedResult() {
      return this.$store.getters.AdminDeletedResult
    }
  },
  watch: {
  ...
  methods: {
    searchAdminList() {
      this.$store.dispatch('actAdminList')
    },
    onClickAddNew() {
      this.$store.dispatch('actAdminInputMode', 'insert') // 모달을 띄운다.
      this.$store.dispatch('actAdminInit')
      this.$bvModal.show('modal-admin-inform')
    },
    onClickEdit(id) {
      // (수정을 위한)상세정보

      // 1. 입력모드 설정
      this.$store.dispatch('actAdminInputMode', 'update')

      // 2. 상세정보 호출
      this.$store.dispatch('actAdminInfo', id)
    ...
    onClickDelete(id) {
      ...
      this.$bvModal.msgBoxConfirm('삭제 하시겠습니까?').then(value => {
        if (value) {
          this.$store.dispatch('actAdminDelete', id)
        }
      })
    }
  }
