    ...
    if circles is not None:
        ...
        if circles is not None:
            ...
            if (len(circles) > 0) and (len(circles) <= 6):
                cv2.putText(mask, "GREEN: " + str(len(circles)), topLeftCornerOfText, font, fontScale, fontColor,
                            lineType)
        else:
            # try white
            ...
            if circles is not None:
                ...
                circles = np.round(circles[0, :]).astype("int")
                if (len(circles) > 0) and (len(circles) <= 6):
                    cv2.putText(mask, "WHITE: " + str(len(circles)), topLeftCornerOfText, font, fontScale, fontColor,
                                lineType)
